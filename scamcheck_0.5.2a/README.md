...
2. **Make sure Python or uv is installed.** Mac and Linux almost always
   already have Python. On Windows, if you're not sure, just try step 3 —
   the launcher checks for [uv](https://docs.astral.sh/uv/getting-started/installation/)
   first (if it's installed, it downloads the correct Python version for you
   automatically — no separate Python install needed) and falls back to a
   regular Python install otherwise, linking you straight to the download
   (https://www.python.org/downloads/) if neither is found. During a manual
   Python install, check the box that says "Add python.exe to PATH."
...
Built for the OUPI Cyber Clinic Contest 2026 by Ryan (Nerdiate), University
of Oklahoma.
